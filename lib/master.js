...
//assignment is set at /lib/api/start_workers
    ...
    function killSignal(signal){
        var options = {SIGTERM: true, SIGKILL: true, SIGINT: true};
        return options[signal] ? options[signal] : false;
    }

    cluster.on('exit', function (worker, code, signal) {
        var shouldTerminate = killSignal(signal);
        if (!shuttingdown && !shouldTerminate) {
            console.log('worker', worker);
            console.log('code', code);
            console.log('signal', signal);
            console.log('process env',process.env.worker, process.env.slicer)
            ...
            console.log('this is the newWorker', newWorker);
            console.log('is this showing up here',cluster.workers[newWorker.id]);
        }
    });
