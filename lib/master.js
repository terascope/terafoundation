...
module.exports = function (context, config) {
    ...
    function determineWorkerENV(context, worker){
        var options = {};

        if(context.descriptors){
            _.forOwn(context.descriptors, function(value, key){
                if (worker.process.env[key]) {
                    options[key] = true;
                }
            });
        }
    ...
    cluster.on('exit', function (worker, code, signal) {
        if (!shuttingdown) {
            ...
            var envConfig = determineWorkerENV(context, worker);
